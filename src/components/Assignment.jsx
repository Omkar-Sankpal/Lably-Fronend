...
const Assignment = ({a_id, title}) => {
  ...
  return (
    ...
    className='border border-primary/10 bg-base-300 m-1 max-w-3xl w-full flex rounded-lg hover:bg-secondary/5 cursor-pointer'>
        <div className='font-medium text-sm md:text-base p-2 border border-primary/10 text-center max-w-[140px] w-full rounded-lg'>
            {a_id || "0000"}
        </div>
        <div className='font-medium p-2 text-sm md:text-base border border-primary/10 w-full rounded-lg text-center'>
            {title || "title"}
        </div>
    </motion.div>
  )
