...
const Student = () => {
  ...
  // Ensure data exists
  const student = ind_stud?.[0]; // first (and only) student object
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    const isCompleted = a.status === true || a.status === 1;
    const m1 = a.marks1 ?? 0;
    const m2 = a.marks2 ?? 0;
    ...
    if (isCompleted) {
      ...
      if (a.a_id.startsWith('CGL')) {
        cgl++;
        cglMarks += m1 + m2;
      } else if (a.a_id.startsWith('DMSL')) {
        dmsl++;
        dmslmarks += m1 + m2;
      } else if (a.a_id.startsWith('TUT')) {
        em++;
        emmarks += m1 + m2;
      } else if (a.a_id.startsWith('PSDL')) {
        psdl++;
        psdlmarks += m1 + m2;
      } else if (a.a_id.startsWith('PBL')) {
        pbl++;
        pblmarks += m1 + m2;
      }
    }
  ...
  return (
    <div className='flex flex-col'>
      <div className='flex flex-col justify-around mt-20'>
        <h2 className='text-2xl font-semibold text-primary text-center'>{student?.student_name ?? "Unknown"}</h2>
        <h4 className='font-medium text-secondary mt-4 text-center'>{student?.roll_no ?? "-"}</h4>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <PieChartComponent completed={completed} remaining={assignments.length - completed} title="Overall Progress" marks={totalMarks} totalmarks={completed * 10} subject="" />
        <PieChartComponent completed={psdl} remaining={14 - psdl} title="PSDL Progress" marks={psdlmarks} totalmarks={psdl * 10} subject="PSDL" />
        <PieChartComponent completed={dmsl} remaining={12 - dmsl} title="DMSL Progress" marks={dmslmarks} totalmarks={dmsl * 10} subject="DMSL" />
        <PieChartComponent completed={cgl} remaining={8 - cgl} title="CGL Progress" marks={cglMarks} totalmarks={cgl * 10} subject="CGL" />
        <PieChartComponent completed={em} remaining={6 - em} title="EM Progress" marks={emmarks} totalmarks={em * 10} subject="TUT" />
